...
static int wav_converter_init(FILE *in_file, FILE *out_file, struct wav_format *format, int length_modification_q10)
{
    ...
    if(fread(chunkID, sizeof(chunkID)-1, 1, in_file) != 1){
        avs_error("audio_effect: Cannot read file \n");
        return -1;
    }
    chunkID[4] = '\0';
    if(strcmp(chunkID,"RIFF")!=0){
        avs_error("audio_effect: chumkID = %s expected RIFF \n", chunkID);
        return -1;
    }

    if(fwrite(chunkID, sizeof(chunkID)-1, 1, out_file) != 1){
        avs_error("audio_effect: Cannot write file \n");
        return -1;
    }
    uint32_t ChunkSize;
    ...
    if(strcmp(Format,"WAVE")!=0){
        avs_error("audio_effect: Format = %s expected WAVE \n", Format);
        return -1;
    }
                        ...
                        void *arg)
{
    ...
    if( in_file == NULL ){
	    avs_error("Could not open file for reading \n");
        return -1;
    }
    out_file = fopen(wavOut,"wb");
    if( out_file == NULL ){
        avs_error("Could not open file for writing \n");
        fclose(in_file);
        return -1;
    }
    ...
    if(ret != 0){
        avs_error("aueffect_alloc failed \n");
        fclose(in_file);
        fclose(out_file);
        return ret;
    }
